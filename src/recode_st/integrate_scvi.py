...
def scVI_integration(config, adata_combined, module_dir):
    ...
    sc.pl.umap(
        ...
        frameon=False,
        ncols=1,
        save=f"_{config.module_name}_scvi_umap.svg",
    )
...
def scANVI_label_transfer(config, adata_combined, scvi_model, module_dir):
    ...
    sc.pl.umap(
        ...
        frameon=False,
        ncols=1,
        save=f"_{config.module_name}_scanvi_umap.svg",
    )
...
def extract_predictions_and_visualize(adata_combined, scanvi_model, adata, module_dir):
    ...
    plt.ylabel("Observed")
    plt.savefig(
        module_dir / "scanvi_confusion_matrix.svg", dpi=300, bbox_inches="tight"
    )
    ...
    # Save plot
    plot_path = module_dir / "scanvi_integration_results.svg"
