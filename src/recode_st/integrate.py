...
def perform_ingest_integration(config, adata_ref, adata, adata_ingest):
...
def scVI_integration(config, adata_ref, adata):
...
# def scANVI_label_transfer(adata_combined, scvi_model, max_epochs_scanvi=100):
#     """Integrates STx data with a reference scRNA-seq dataset using scVI.
...
#     LABELS_KEY = (
#         "training_cell_type_scanvi"  # new column that scANVI will use training labels.
#     )
#     UNLABELED_CATEGORY = (
#         "Unknown"  # placeholder for cells that do not have known labels aka STx cells
#     )

#     # Set the label column (LABELS_KEY) to "Unknown" for all cells initially.
...
def run_integration(config: IntegrateModuleConfig, io_config: IOConfig):
    ...
    adata_combined, scanvi_model = scVI_integration(config, adata_ref, adata)

    logger.info(f"Model training complete. adata combined: {adata_combined}")
    logger.info(f"Model training complete. Model: {scanvi_model}")
    print(cmap)  # prevent error message
