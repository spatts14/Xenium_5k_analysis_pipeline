...
def visualize_drug2cell(config, adata, annotation: str = ANNOTATION, cmap=None):
    ...
    sc.pl.umap(
        ...
        color=drug_list,
        color_map=cmap,
        save=f"_{config.module_name}_drug2cell_umap.svg",
    )
    ...
    for roi in adata.obs["ROI"].unique():
        ...
        sq.pl.spatial_scatter(
            ...
            wspace=0.4,
            cmap=cmap,
            save=f"_{config.module_name}_{roi}_scatter.svg",
        )
...
def calc_n_plot_DE(adata, config, annotation: str = ANNOTATION, cmap=None):
    ...
    sc.pl.rank_genes_groups_dotplot(
        ...
        n_genes=3,
        cmap=cmap,
        save=f"_{config.module_name}_{ANNOTATION}_dotplot.svg",
    )
...
def run_drug2cell(config: Drug2CellModuleConfig, io_config: IOConfig):
    ...
    sc.pl.dotplot(
        ...
        swap_axes=True,
        cmap=cmap,
        save=f"_{config.module_name}_{ANNOTATION}_all_drugs_by.svg",
    )
    ...
    if len(valid_groups) >= 2:
        ...
        sc.pl.dotplot(
            ...
            **plot_args,
            cmap=cmap,
            save=f"_{config.module_name}_{ANNOTATION}_respiratory_drugs_by.svg",
        )
    else:
        logger.warning(
            f"Not enough groups with >= {min_cells_per_group} cells "
